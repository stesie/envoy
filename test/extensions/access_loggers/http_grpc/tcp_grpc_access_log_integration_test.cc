...
public:
  TcpGrpcAccessLogIntegrationTest() : BaseIntegrationTest(ipVersion(), ConfigHelper::TCP_PROXY_CONFIG) {
  ...
  AssertionResult waitForAccessLogRequest(const std::string& expected_request_msg_yaml) {
    ...
    log_entry->mutable_common_properties()->clear_upstream_remote_address();
    log_entry->mutable_common_properties()->clear_upstream_local_address();
